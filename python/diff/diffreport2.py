...
class  Report():
    ...
    def diff(self,url,a,param1):
        self.diff_count += 1
        self.mailtext = self.mailtext + '[' + str(self.counter) + ']-----------<button   id="btn' + str(self.diff_count) + '" onclick="aaa(\'btn' + str(self.diff_count) + '\',' + str(self.diff_count) + ')">>>>展开</button><button   id="to_btn' + str(self.diff_count) + '" onclick="move_to_diff(' + str(self.diff_count) + ')">>>>跳转到第一处不同</button><br>'
        ...
        self.mailtext = self.mailtext + '<div class="left" style="height: 100px; overflow:scroll"  id="div' + str(self.diff_count) + 'con1"  onscroll="move(' + str(self.diff_count) + ',2,1)"><ol> <pre id="div' + str(self.diff_count) + 'pre1">' + str(json.dumps(a[1], ensure_ascii=False)) + '</pre></ol></div>'
        self.mailtext = self.mailtext + '<div class="left" style="height: 100px; overflow:scroll"  id="div' + str(self.diff_count) + 'con2"  onscroll="move(' + str(self.diff_count) + ',1,2)"><ol> <pre id="div' + str(self.diff_count) + 'pre2">' + str(json.dumps(a[2], ensure_ascii=False)) + '</pre></ol></div>'
    ...
    def end(self):
        self.mailtext = self.mailtext + '<script type="text/javascript"> var onOff=false; var ip =[]; var ids = document.getElementsByName("window"); for(var i=0;i<ids.length;i++) { ids[i].style.display="none"; }function move(a,b,c){ document.getElementById("div"+a+"con"+b).scrollTop = document.getElementById("div"+a+"con"+c).scrollTop; document.getElementById("div"+a+"con"+b).scrollLeft = document.getElementById("div"+a+"con"+c).scrollLeft; } function aaa(e,f) { var btn=document.getElementById(e); if(onOff==true) { document.getElementById("div"+f).style.display="none"; btn.innerHTML=">>>展开" } else { document.getElementById("div"+f).style.display="block"; btn.innerHTML ="<<<收缩"} onOff=!onOff; if (ip[f] != f) { var obj1 = document.getElementById("div"+f+"pre1").innerText; var obj2 = document.getElementById("div"+f+"pre2").innerText; var str1 = JSON.stringify(JSON.parse(obj1), null, 4); var str2 = JSON.stringify(JSON.parse(obj2), null, 4); output(ShowColor(str1,f),f); output2(ShowColor(str2,f),f); ip[f]=f; } return false; } function output(inp,f) { document.getElementById("div"+f+"pre1").innerHTML= inp; } function output2(inp,f) { document.getElementById("div"+f+"pre2").innerHTML= inp; } function ShowColor(json,f){ var a1 = \'<span id="span\'+f+\'"  name="span\'+f+\'" style=background:yellow;>\'; var a2 =\'<span id="span\'+f+\'"  name="span\'+f+\'" style=background:yellow;>\'; var a3 = "</span>"; return json.replace(/ThisIdYellowBegin/g, a1).replace(/ThisIdYellowEnd/g, a3).replace(/ThisIdPinkBegin/g, a2).replace(/ThisIdPinkEnd/g, a3); }function move_to_diff(f){ ele = document.getElementById("span"+f);if (ele != null){ele.scrollIntoView();ele.id = "spana";}else{alert("跳转已到底了");}} </script> </body> </html>'
        ...
        with io.open("//Users/zhaocuixia/ajaxdiff3.html", "w", encoding='utf-8') as f:
